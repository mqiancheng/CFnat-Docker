...
```shell
docker run -d --name mycfnat --restart always -p 1234:1234 mqiancheng/cfnat
```

- 镜像仓库拉取

```shell
docker run -d --name mycfnat --restart always -p 1234:1234 docker.fxxk.dedyn.io/mqiancheng/cfnat
```
...
- IP类型(ips): 6
```shell
docker run -d -e colo="HKG" -e delay=160 -e ips=6 --restart always -p 1234:1234 mqiancheng/cfnat
```
...
- **本地映射端口: 8080**
```shell
docker run -d -e colo="HKG" -e delay=160 -e ips=4 -e port=80 -e tls=false --restart always -p 8080:1234 mqiancheng/cfnat
```
...
- IP类型(ips): 4
```shell
docker run -d -e colo="SJC,LAX" -e delay=300 -e ips=4 --restart always -p 1234:1234 mqiancheng/cfnat
```
